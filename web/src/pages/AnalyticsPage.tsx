              ...
              <tbody>
                {data.sources.map((entry) => (
                  <tr key={entry.provider} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="py-2 px-4"><Badge tone="outline">{entry.scope}</Badge></td>
                    <td className="py-2 px-4"><Badge tone="secondary">{entry.unit}</Badge></td>
          ...
          {data.unsupported.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {data.unsupported.map((entry) => (
                <Badge key={entry.provider} tone="outline">{entry.provider}</Badge>
              ))}
            </div>
          )}
        </div>
