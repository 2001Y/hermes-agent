...
_SKILL_REVIEW_PROMPT = (
    ...
    "Read-before-write (ENFORCED for existing-file writes): before "
    "patch/edit of an existing SKILL.md, load the exact target in "
    "THIS review turn with skill_view(name). Before overwrite, patch, "
    "or remove of an existing supporting file, call "
    "skill_view(name, file_path=...). Content quoted earlier in the "
    "conversation does NOT satisfy this guard. A new skill or new "
    "supporting file does not require (and cannot have) a pre-read. If "
    "skill_manage returns _read_before_write_required, view that exact "
    "target and retry once, basing the write on the content just "
    "returned. Touching multiple existing files? Handle them one at a "
    "time: view, write, then move to the next.\n\n"
